...
from wtforms import StringField, PasswordField, BooleanField, SubmitField, SelectMultipleField, SelectField, ValidationError, TextField
...
class DepartForm(Form):
	...
	def validate_department(self, field):
		if request.endpoint[:20] != 'admin.departs_modify':
...
class UserForm(Form):
	...
	def validate_email(self, field):
		if request.endpoint[:18] != 'admin.users_modify':
...
class AssetForm(Form):
	...
	def validate_domain(self, field):
		if request.endpoint[:18] != 'src.assets_modify':
